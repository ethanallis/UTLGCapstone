...
This document contains instructions for settings up the Ubuntu 20.04 LTS base configuration test lab by deploying two servers running Ubuntu Server, one client running Ubuntu Desktop and one firewall built on Ubuntu Server. Once completed, the lab will simulate the functioning of a private intranet.  
...
.. At least two cores and two gigabtyes of RAM is reccomended for Ubuntu 20.04 LTS machines
...
. Navigate to the section labed `// forwarders {`
...
.. Uncomment everything under that desription tag down to the white space before the next description tag
. Change the following lines to the dhcpd file so that it looks similar:
+
...
.. Select `Continue`
. Select `Erase disk and install Ubuntu` for the Instalaltion type, then select `Install Now`
.. Select `Continue` when prompted regarding overwriting the drive 
...
. Navigate to a web browser and browse to https://www.splunk.com/en_us/download/splunk-enterprise.html
.. This can be done on any web browser inside or outside of the environemt 
.. You also must create or log into an account
. Navigate to the Linux section and download the `.deb` version
.. Select `Download Now`
.. Copy or make note of the wget command located under `Useful Tools` in the top right
...
```
. Run the aformentioned wget command on the App Server machine
.. `sudo wget -O splunk-8.2.2.1-ae6821b7c64b-linux-2.6-amd64.deb...`
. Unpack Splunk using the following command:
.. `sudo dpkg -i splunk-8.2.2.1-ae6821b7c64b-linux-2.6-amd64.deb`
. Start Splunk and accept the license using the following command:
.. `sudo /opt/splunk/bin/splunk start --accept-license`
.. Create a splunk administratior user
... For this test purpose we will re-use our champuser credentials, something that should never be done in a production environment
. Stop Splunk using the following command:
.. `sudo /opt/splunk/bin/splunk stop`
. Enable splunk to run at boot as the splunk user with the following command:
.. `sudo /opt/splunk/bin/splunk enable boot-start -user splunk`
. The ownership of the /opt/splunk files mist be changed to the splunk user
.. `sudo chown -R splunk:splunk /opt/splunk`
. Start splunk again with `sudo /opt/splunk/bin/splunk start`
. Allow the following ports through the local firewall with the following commands:
.. `sudo ufw allow from any to any port 8000 proto tcp`
.. `sudo ufw allow from any to any port 9997 proto tcp`
. The splunk web portal can now be accessed by navigateting to the hostname of the App Server at port 8000
.. http://app1.college.com:8000 or http://192.168.100.20:8000
.. This can be done on client1, which has a GUI and web browser
. Upon navigateing to the splunk web portal you should be greeted with a login page
.. Enter your credentials and you should be successfully taken to the Splunk landing page like the one shown below
+
image::splunklogin.jpg[Splunk Login, align="center"]

=== Splunk Universal Forwarder Configuration

. The following commands are to be run on the machine in your environment you wish to monitor
. Navigate to a web browser and browse to https://www.splunk.com/en_us/download/universal-forwarder.html
.. This can be done on any web browser inside or outside of the environemt 
.. You also must create or log into an account
. Navigate to the Linux section and download the `.tgz` version
.. Select `Download Now`
.. Copy or make not of the wget command located under `Useful Tools` in the top right
.. In this case the command is as follows
```
...
```
. Run the aformentioned wget command on the server to be monitored
.. `sudo wget -O plunkforwarder-8.2.2.1-ae6821b7c64b-Linux-x86_64.tgz...`
. Uncompress the universal forwarder to the /opt/ directory using the followng command:
.. `sudo tar xvzf splunkforwarder-8.2.2.1-ae6821b7c64b-Linux-x86_64.tgz -C /opt`
. Start Splunk Universal Forwarder and accept the license using the following command:
.. `sudo /opt/splunkforwarder/bin/splunk start --accept-license`
.. Create a splunk administratior user
... For this test purpose we will re-use our champuser credentials, something that should never be done in a production environment
. Add a splunk forward server to app1 using the following command:
.. `sudo /opt/splunkforwarder/bin/splunk add forward-server 192.168.100.20:11000`
. Allow port 11000 TCP through the local firewall with the following command:
.. `sudo ufw allow from any to any port 11000 proto tcp`
. Add a local monitor for the files you want to see on splunk hosted by app1 using the following command:
.. `sudo /opt/splunkforwarder/bin/splunk add monitor /var/log/...`
. Restart Splunk using `sudo /opt/splunkforwarder/bin/splunk restart`
. On the splunk web porta navigate to the Settings > Forwarding and receiving tab, and configure recieving
.. Add port 9997 as a receiving port 
. Splunk should start revicing logs from the directories being moitored on the remote machine
