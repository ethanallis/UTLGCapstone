...
== Security Service
The Wazuh agent is multi-platform and runs on the hosts that the user wants to monitor. It communicates with the Wazuh manager, sending data in near real time through an encrypted and authenticated channel. In this case we're installing the Wazuh Agent on OpenBSD to send logs to the Wazuh Manager which is installed on Ubuntu.
...
. Next open up and edit the `/var/ossec/etc/ossec.conf` file again and add the following lines below the last <wodle>:
+
```
...
```
This will enable running vulnerability scans on the agent.

. In the same file `/var/ossec/etc/ossec.conf` add the following lines to enable Active Response.
+
```
<active-response>
  <disabled>no</disabled>
...
. Using the same config file you can change the following lines to change the scanning options for File Integrity.
+
Here you can change the directories which can be scanned/monitored.
```
<syscheck>
...
```
Here you can change the frequency of the scans in the selected directories. The 36000 is counted in seconds which equals to 10 hours.
```
<syscheck>
...
```
You can all change it to a certain time and day which the scan should start at.
```
<syscheck>
